...
warning off;
disp('-------------------------------------------------------------------------------------')
disp('                 Welcome to the ATMOZ uncertainty code for ARE2017 Campaign             ')
disp('                     Triad Brewers and dates: 29/05/2017 -- 7/06/2017                     ')
...
% load the input of the Brewer number
	...
	% Figure 2 - Ozone Relative uncertainty
	figure(2)
...
% uncertainty of the PMT temperature (the resolution of the PMT is 1�C)
u_temp = 1/sqrt(3); 
...
% Neutral filter coefficients and its uncertainties of the ARE2017 campaign
...
meas = N_meas;
o3_meas = (meas-etc-m_5.*pre_5.*B/1013.25)./(10*A.*mu_5);

% O3 without Rayleigh correction
o3_meas_r = (meas-etc)./(10*A.*mu_5);
%%
...
b = [t_ref' Omega_ref' mu_ref' sza_ref' ms9_ref' u2_ms9_ref' uo3_ref' uo3_ref'];
...
if DS_num == 1
	u2_etc_bp = (10*A(1)*dat(:,4).*(dat(:,13)+t_etc.*diff_o3')).^2 + (10*dat(:,8).*dat(:,4)*u_A_bp).^2 + mean(u2_mu_5)*(10*dat(:,8).*A(1)).^2 + dat(:,7);
	u_etc = median(sqrt(u2_etc_bp))./sqrt(length(dat(:,4)));
elseif DS_num == 2
	u2_etc_sg = (A(1)*dat(:,4).*(dat(:,13)+t_etc.*diff_o3')).^2 + (dat(:,8).*dat(:,4)*u_A_sg).^2 + mean(u2_mu_5)*(dat(:,8).*A(1)).^2 + dat(:,7);
...
c = u2_mu_5.*(10*Omega_5.*A).^2 + (10*mu_5.*Omega_5.*u_A).^2 + ((B.*pre_5/Pstan).^2).*u2_m_5 + (m_5.*(pre_5/Pstan).*u_B).^2 +...
    ((m_5.*B./Pstan).^2).*u_pre.^2 + u_etc.^2 + 2*100*(Omega_5.^2).*A.*mu_5.*sqrt(u2_mu_5).*u_A.*rho(:,1,3)' +...
    ...
    2*(B.*pre_5.*m_5.*m_5./(Pstan^2)).*u_pre.*u_B.*rho(:,4,6)' - 2*10*Omega_5.*A.*u_etc.*sqrt(u2_mu_5).*rho(:,3,7)'-...
    2*10*mu_5.*Omega_5.*u_etc.*u_A.*rho(:,1,7)' - 2*B.*(pre_5/Pstan).*u_etc.*sqrt(u2_m_5).*rho(:,5,7)'-...
...
%S2_o3 = (u_etc./(10*A.*mu_5)).^2 + u2_xx_5.*(1./(10*A.*mu_5)).^2 + u2_m_5.*((pre_5.*B/Pstan)./(10*A.*mu_5)).^2 + (u_pre.*(m_5.*B./Pstan)./(10*A.*mu_5)).^2 +...
%(u_B.*(m_5.*pre_5./Pstan)./(10*A.*mu_5)).^2 + (u_A.*(etc-MS9_r-m_5.*(pre_5/Pstan).*B).*(10*mu_5)./((10*A.*mu_5).^2)).^2 +...
%u2_mu_5.*((etc-MS9_r-m_5.*(pre_5/Pstan).*B).*(10*A)./((10*A.*mu_5).^2)).^2;

figure(23)
plot(t_ref,100*uo3_ref./Omega_ref,'*',t_j_5,100*uo3./Omega_5,'+')
datetick('x',dateFormat)
set(gca,'Ylim',[0,20])
title('Ozone relative uncertainty');
xlabel('Time')
...
legend('185','other')
grid on
%plot(t_j_5,100*sqrt(S2_o3)./Omega_5,'+')
grid on