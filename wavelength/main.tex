...
%%f
\begin{figure}[t]
\includegraphics[width=12cm]{opo.eps}
...
%%f
\begin{figure}[t]
\includegraphics[width=12cm]{General_NoLinearity_corr.eps}
...
\begin{figure}[t]
\includegraphics[width=8.3cm]{figures/General_002_central_comparison.png}
...
\begin{figure}[t]
\includegraphics[width=8.3cm]{figures/General_002_fwhm_comparison.png}
...
\tophline
     & brw\_scan^1  & opo\_quad^2 & opo\_cubic^3 & lamp\_quad^4 & lamp\_cubic^5 \\
\middlehline
...
\begin{enumerate}
    ...
    \item Laser wavelength scanned @ fixed Brewer grating position 
    \item Brewer grating position. changed @ fixed laser wavelength, dispersion approx.  by a quadratic function 
    \item Brewer grating pos. changed @ fixed laser wavelength, dispersion approx.  by a cubic function 
    \item Brewer grating pos. changed @ fixed lamp emission wavelength, dispersion approx.  by a quadratic function 
    \item Brewer grating pos. changed @ fixed lamp emission wavelength,   dispersion approx.  by a cubic function  % Table Footnotes
\end{enumerate}
}
\label{tab:o3abs_sum}
\end{table*}
...
%\item 
 %We don't find significant differences when different input optics is used for the wavelength characterization, placing the spectral source on the global port, direct port or using the Hg internal lamp are interchangeable for this purposes.
    ...
    \item The quadratic dispersion relation fit used in the standard Brewer algorithm is not suitable outside the ozone spectral range 310 \unit{nm} - 320 \unit{nm}. The residuals show a systematic pattern, which is particularly important in the higher end of the spectral range.%-> effect o the ozone<-
