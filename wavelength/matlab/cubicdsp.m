 function [fwl,fstps,pwl,pstps]=cubicdsp(wl,A)
...
for i=1:6,%2:7,
   ...
   if sum(ind)>2,
    ...
    p=polyfit(A(ind,i),wl(ind),3);
    p2=polyfit(wl(ind),A(ind,i),3);
...
function [p,s,v]=polyfit3(x,y)
[p1,s,m]=polyfit(x,y,3);
%desacemos el cambio
p(1)=p1(1)/m(2)^2;
p(2)=p1(2)/m(2)-2*p1(1)*m(1)/m(2)^2;
p(3)=p1(1)*(m(1)/m(2))^2-p1(2)*(m(1)/m(2))+p1(3);
p(4)=p1(1)*(m(1)/m(2))^3-p1(2)*(m(1)/m(2))^2+p1(3)*(m(1)/m(2))+p1(4);
