function [wl,DSP,DSPstd,fwhm,fwhmstd,backlash,resup,names]=alldsp_dev(day,year,brew,lines,minslit,maxslit,usewl,dspp)
...
if nargin<4,lines=1:15;end
if isempty(lines),lines=1:30;end
...
if isempty(dspp),
    dspp=[ sprintf('%03d',brew) '_' sprintf('%02d',year) '_' sprintf('%03d',day(1)) filesep() ];
    %dspp='.';
end
if ~ischar(brew),
 ...
 for lnes=lines,
    ...
    for days=day,
            ...
            if lnes>9
             ...
             fmt=['W',char(55+lnes),'%d%03d%02d%s'];
             filename=sprintf(fmt,slits,days,year,brew);
            end
            ...
            if isempty(a),
                %disp(['file '  fullfile(dspp,filename) ' does not exist']);
            else
                disp(['now:' filename,sprintf(' %d %f',slits,str2num(s(1,:)))]);
                file_name=filename;
                daycnt=daycnt+1;
                figcnt=figcnt+1;
                if (rem(figcnt,7)==0)
                ...
                %n=a(:,2)./monitor_brewer(a(:,2)/(4*0.1144))';
                n=a(:,2)./monitor_brewer_c(a(:,2)/4)';
                %n=a(:,2);
                n=[a(:,1),n];
                [dsp_arrU(lnes,slits+1),dsp_arrD(lnes,slits+1),tempfw,resup(lnes,slits+1)]=dsp(n,s,0.2,0.8,1);
                if isnan(tempfw)
                    disp('w fail');
                     [dsp_arrU(lnes,slits+1),dsp_arrD(lnes,slits+1),tempfw,resup(lnes,slits+1)]=dsp(n,s,0.1,0.9,1);
                end
