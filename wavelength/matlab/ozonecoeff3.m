...
mic_range=[1020
        ...
        7711];
 old=oposrange;
    oposrange=(mic_range+mic_ozonepos-cal_ozonepos)';
...
for opos=oposrange;
        ...
        if sum(ind)>1,
           ...
           try
            [buf]=brslit(fwhmwl(i),o3x1wl-thiswl(i)); % calculate slitfunction
             ...
             salida{jj}.slit{i}=buf'*NaN;
             o3coeff(i)=NaN; % integrate
             o3coeff(i)=NaN; % need in log10 units, as in brewer
           end    
    ...
    resumen_=[opos-mic_ozonepos+cal_ozonepos,O3Coeff,RAYCoeff*1e4,So2coeff,O34So2cc,-I0Coeff,O3Daumont];
    resumen=[resumen;resumen_];
    salida_m(:,:,jj)=[thiswl;fwhmwl;o3coeff;raycoeff;so2coeff;I0];                     
    
    s1=sprintf('%5.0f WL(A)        %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f',opos-mic_ozonepos+cal_ozonepos,thiswl);
