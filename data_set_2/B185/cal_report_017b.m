...
%    if jj==length(ldsp),confign=2; else confign=1; end
    try
      [res{jj},detail{jj},DSP_QUAD{jj},QUAD_SUM{jj},QUAD_DETAIL{jj},...
       ...
       ]=dspreport(Cal,'dsp_dir',fullfile(Cal.path_root,'..','DSP',ldsp{jj}),'config_n',1);%
    catch
       warning(sprintf('Error en %s. DSP: %s',Cal.brw_name{Cal.n_inst},ldsp{jj}));
       res{jj}=NaN*ones(15,9,2); detail{jj}=NaN*ones(6,6,9,2); QUAD_DETAIL{jj}=NaN;
       %cd ..
    end
    disp('x');
...
%%
tabla_dsp=report_dispersion(Cal,'grp','month','fpath',fullfile(Cal.path_root,'..','DSP'),...
                                           'date_range',datenum(2014,7,1):datenum(2017,3,1));  
dsp_017=array2table((tabla_dsp.data(:,[1,15,17,20,21,22,23])),'VariableNames',varname(tabla_dsp.data_lbl([1,15,17,20,21,22,23])));
dsp_017(isnan(dsp_017.A1Quad_),:)=[];
dsp_017=table2timetable(dsp_017,'RowTimes',date_time(dsp_017.Date))
%save(Cal.file_save,'-APPEND','dsp_summary');
%% stats
sds=cell2mat(cellfun(@(x) [x(end-1,1,1);x(end-1,2,1);x(end-1,2,2)]',res,'UniformOutput',false)');
sum_dsp=[dates,sds]
ploty(sum_dsp(:,[1,3:4]),'-o');
grid;
datetick;
mds=nanmedian(sum_dsp)
%%
