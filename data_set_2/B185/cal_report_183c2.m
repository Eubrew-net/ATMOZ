% options_pub.outputDir=fullfile(pwd,'latex','183','html'); options_pub.showCode=true;
% close all; publish(fullfile(pwd,'cal_report_183c2.m'),options_pub);
...
t_183=table2timetable(t_atmoz{Cal.n_inst,2},'RowTimes',date_time(t_atmoz{Cal.n_inst,2}.Date));
writetable(timetable2table(t_183),'B183_atmoz.xls')

% test
%o3x=(t_183.ms9_corr-t_183.ETC)./(t_183.A1.*t_183.m2*10);
%plot(o3x-t_183.O3_1)
meteo=table2timetable(readtable('atmoz_mteo_eff.xls'));
ts=synchronize(t_183,meteo,'first','nearest','EndValues',NaN);
...
t_183.MS9_2=MS9;
t_183.p=ts.pressure_hPa_;
t_183.T_eff=ts.Teff_ecc;
...
ref=[]; 
ref=sortrows([summary{1};summary{3}],1);
...
%final
    [x,r,rp,ra,dat,ox,osc_smooth_alt]=ratio_min_ozone(...
...
h=plot_smooth(osc_smooth_ini,osc_smooth_alt);
legend(h,'Config. initial','Final configuration','cal'); set(gca,'YLim',[-4 2]);
