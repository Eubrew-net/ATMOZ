function Options=printfiles_publication(n0,patern,varargin)
...
Options.DefaultFixedFontSize=9; %a positive scalar in 'fixed' mode specified the default font size in points
Options.FontSizeMin=8;           %a positive scalar specifies the minimum font size allowed after scaling
Options.FontSizeMax=14;          %a positive scalar specifies the maximum font size allowed after scaling
...
%         end
     ...
     saveas(h,figura,'fig');
     eps2pdf([figura,'.eps'],[figura,'.pdf'],true,false,false,101)
     %saveas(h,figura,'pdf');
end
cd(cwd);
catch exception
