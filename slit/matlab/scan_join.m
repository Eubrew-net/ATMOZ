...
% a�ade el scan1 (lamda1,medidaN+1) SIMPLE a la matriz de 
...
if isempty(d) & isempty(d_0) % son iguales los scanes               
    ...
    scan=[I,scan0(il,2:end),scan1(il_0,2:end)]; %J looks like there was an error in the previous line?
...
elseif isempty(d)               % 2 esta inclido en 1
    ...
    %aux0(il,:)=scan0(il,2:end); %J looks like there was an error in the previous line?
    scan=[u,aux0,aux1];
    %warning('ss')
end     
