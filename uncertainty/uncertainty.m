...
disp('--------------------------------------------------------------------------')
disp('                 Welcome to the ATMOZ uncertainty code (v4.0)             ')
...
for j = 2:length(O3L1)
	...
	f0(j-1) = cell2num(O3L1{1,j}(19)); % corrected measurement of the lambda0
	f1(j-1) = cell2num(O3L1{1,j}(20)); % corrected measurement of the lambda1
	f2(j-1) = cell2num(O3L1{1,j}(21)); % corrected measurement of the lambda2
	f3(j-1) = cell2num(O3L1{1,j}(22)); % corrected measurement of the lambda3
	f4(j-1) = cell2num(O3L1{1,j}(23)); % corrected measurement of the lambda4
	f5(j-1) = cell2num(O3L1{1,j}(24)); % corrected measurement of the lambda5
...
for k = 2:length(BH)
	A1(k) = cell2num(BH{1,k}(12))*10; % Ozone absorption coefficient (units?)
	...
	Tcoef2(k) = cell2num(BH{1,k}(7)); % Temperature coefficient of lambda2
	Tcoef3(k) = cell2num(BH{1,k}(8)); % Temperature coefficient of lambda3
	Tcoef4(k) = cell2num(BH{1,k}(9)); % Temperature coefficient of lambda4
	Tcoef5(k) = cell2num(BH{1,k}(10)); % Temperature coefficient of lambda5
...
for i=1:length(period)
	dates(i,:)
	url_str=[url_base,url_ds,'?brewerid=',brewer_str,'&date=',dates(i,:)]
	curl_str=['curl -s --connect-timeout 120 "',url_str,'"'];
	[status,data_json]=system(curl_str);
...
for k = 2:length(DS)
	ccl(k-1) = cell2num(DS{1,k}(9)); % number of cicles
...
% the temperature coefficients and its uncertainties from Huelva 2017
[tc,v,brw] = tblread('temperature_huelva2017.csv',',');
...
utc2 = tc(ind,9); % uncertainty of the temperature coefficient of lambda2
utc3 = tc(ind,10); % uncertainty of the temperature coefficient of lambda3
utc4 = tc(ind,11); % uncertainty of the temperature coefficient of lambda4
utc5 = tc(ind,12); % uncertainty of the temperature coefficient of lambda5
...
air_m_o = sec(asin((R/(R+heff_o))*sin(Z_rad)));
